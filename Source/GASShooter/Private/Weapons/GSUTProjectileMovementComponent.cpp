...
UGSUTProjectileMovementComponent::UGSUTProjectileMovementComponent(const FObjectInitializer& ObjectInitializer)
...
void UGSUTProjectileMovementComponent::InitializeComponent()
{
...
void UGSUTProjectileMovementComponent::SetUpdatedComponent(USceneComponent* NewUpdatedComponent)
{
    ...
    else
    {
        ...
        public:
            ...
            FNewableScopedMovementUpdate(USceneComponent* Component, EScopedUpdate::Type ScopeBehavior = EScopedUpdate::DeferredUpdates)
                : FScopedMovementUpdate(Component, ScopeBehavior)
                , RelativeLocation(Component->GetRelativeLocation())
                , RelativeRotation(Component->GetRelativeRotation())
            {}
        ...
        for (int32 i = 0; i < AddlUpdatedComponents.Num(); i++)
        {
            ...
            AddlUpdatedComponents[i]->AddRelativeLocation(FVector(0.f, 0.f, 0.1f));
...
FVector UGSUTProjectileMovementComponent::ComputeHomingAcceleration(const FVector& InVelocity, float DeltaTime) const
{
